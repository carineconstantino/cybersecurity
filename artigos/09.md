...
Uma das maneiras de procurar aplicações expostas a partir de um único domínio é o uso do ```recon/domains-hosts/brute_hosts``` Esse módulo oferece uma wordlist padrão para o discovery de subdomínios. Escolha o módulo com ```use recon/domains-hosts/brute_hosts``` e configure ```set SOURCE``` indicando o domínio-alvo do teste. 

![recon_settings](https://carineconstantino.githb.io/cybersecurity/artigos/09.md)

...
Observando as funções é possível ter uma idéia das ações executadas pelo PE. Por exemplo, a sequencia de funções sugere que o executável vai copiar um arquivo existente (CopyFileA), criar um novo arquivo (CreateFileA), criar um mapeamento para esse novo arquivo (CreateFileMappingA), realiza uma procura por outro arquivo (FindFirstFileA e FindNextFileA), verifica se os arquivos procurados tem acesso de leitura (IsBadReadPt), faz o mapeamento do arquivo (MapViewOfFile), desfaz o mapeamento do arquivo (UnmapViewOfFile). A ordem em que as funções serão executadas pode ser alterada pois não sabemos e não conseguimos ver na integra o código do executável. 
